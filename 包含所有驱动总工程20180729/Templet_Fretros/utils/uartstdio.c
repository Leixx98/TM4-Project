...
UARTvprintf(const char *pcString, va_list vaArgP)
{
    ...
    char pcBuf[16], pcStrBuffer[16],cFill;
		char *pcStr=pcStrBuffer;
    //
    // Check the arguments.
    //
    ASSERT(pcString != 0);
					...
					case 'f':
                {
                    ...
                    if((int32_t)value_f<0)
                    {
                        ...
                        value_f=-value_f;
										...
										else if((int32_t)value_f<1)
                        UARTwrite("0", 1);										
                    int_num = (uint32_t)(value_f);
                    dec_num = (uint32_t)((value_f-int_num)*10000);
                    while(int_num)
                    {
                        pcBuf[ui32Count++] = (uint32_t)int_num%10;
                        int_num= int_num/10;
                    }
                    ui32Pos+=ui32Count;
                    while (ui32Count)
                    {
                        *(pcStr++)= pcBuf[--ui32Count] + '0';
                    }
                    *(pcStr++)='.';
                    ui32Pos++;
                    while(dec_num)
                    {
                        pcBuf[ui32Count++] = (uint32_t)dec_num%10;
                        dec_num= dec_num/10;
                    }
                    ui32Pos+=ui32Count;
                    while (ui32Count)
